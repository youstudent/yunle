...
```php
use pd\admin\components\MenuHelper;
...
Return of `pd\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `pd\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `pd\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
```php
<?php
user pd\admin\components\Helper;
