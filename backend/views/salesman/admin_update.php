            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-inverse">
                    ...
                    <div class="panel-body">
                        ...
                        <?php
                        //获取对应的图片数据
                        $heads  = \common\models\UserImg::find()->where(['user_id'=>$model->id, 'status'=> 1, 'type'=> 1])->all();
                        $head_config = [];
                        $head_preview = [];
                        $input = '';
                        foreach($heads as $head){
                            ...
                            $input .= '<input type="hidden"  data-head-img-node="1" id="head_img_id_input_'.$head->id.'" name="UserForm[head_id][]" value="'.$head->id.'">';
                        }

                        $attas = \common\models\UserImg::find()->where(['user_id'=>$model->id, 'status'=> 1, 'type'=> 0])->all();
                        $atta_config = [];
                        $atta_preview = [];
                        foreach($attas as $atta){
                            ...
                            $input .= '<input type="hidden"  data-atta-img-node="1" id="atta_img_id_input_'.$atta->id.'" name="UserForm[atta_id][]" value="'.$atta->id.'">';
                        }
                        ?>
                        ...
                        if(pd\admin\components\Helper::checkRoute('/abs-route/customer-manager')){
                            ...
                            //所属的服务商信息
                            $column = \backend\models\Service::find()->indexBy('id')->where(['id'=>$ids])->select('name,id')->column();
                        }else{
                            //所有服务商
                            $column = \backend\models\Service::find()->indexBy('id')->select('name,id')->column();
                        }

                        ?>

                        <?= $form->field($model, 'pid')->dropDownList($column) ?>
...
$salesman_id = $model->id;
$this->registerJs(<<<JS
$(function () {
    var init_url = "{$url}?service_id=" + $('#userform-pid').val() + '&salesman_id={$salesman_id}';
    $.get(init_url, function(rep){
